...
- [Agentage Memory](https://memory.agentage.io) - One memory. Every AI. Owned by you. Remote (Streamable HTTP) MCP server giving Claude, Cursor, and ChatGPT one shared markdown memory you own and can export anytime. 6 tools: memory__search, memory__read, memory__write, memory__edit, memory__list, memory__delete. Endpoint at `https://memory.agentage.io/mcp` - OAuth 2.1 + PKCE + Dynamic Client Registration, Protected Resource Metadata served (401 on unauth). Listed in the official MCP Registry as `io.agentage/memory`.
