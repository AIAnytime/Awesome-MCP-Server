...
- [Find MCP](https://github.com/agentage/find-mcp) — MCP server for discovering other MCP servers. Searches the agentage MCP Catalog: 17,000+ servers synced from the official MCP registry (registry.modelcontextprotocol.io). Install via `npx @agentage/find-mcp`. Listed on MCP Registry as `io.agentage/find-mcp`.
